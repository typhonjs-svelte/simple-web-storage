...
describe('store', () =>
{
   describe('writable', () =>
   {
      ...
      {
         let num = 0;
         const store = writable('store', num, (set, update) => update((val) => val + 1));

         let count1 = 0;
         let count2 = 0;

         assert.equal('0', globalThis.localStorage.getItem('store'));

         store.subscribe(() => count1 += 1)();
         assert.strictEqual(count1, 1);

         assert.equal('1', globalThis.localStorage.getItem('store'));

         const unsubscribe = store.subscribe(() => count2 += 1);
         assert.strictEqual(count2, 1);

         assert.equal('2', globalThis.localStorage.getItem('store'));
      ...
      it('initial value should be stored', () =>
      {
         ...
         const stored_value = JSON.parse(globalThis.localStorage.getItem('store'));
   ...
   describe('readable', () =>
   {
      it('creates a readable store', () =>
      {
         ...
         const store = readable('store', void 0, (set) =>
         {
            tick = set;
            running = true;
   ...
   describe('derived', () =>
   {
      ...
      it('prevents diamond dependency problem', () =>
      {
         ...
         const a = derived('a', count, ($count) => 'a' + $count);
         const b = derived('b', count, ($count) => 'b' + $count);

         const combined = derived('combined', [a, b], ([a, b]) => a + b);
      ...
      it('derived dependency does not update and shared ancestor updates', () =>
      {
         ...
         const a = derived('a', root, ($root) => 'a' + $root.a);

         const b = derived('b', [a, root], ([$a, $root]) => 'b' + $root.b + $a);
      ...
      it('discards non-function return values', () =>
      {
         ...
         const d = derived('d', num, ($num, set) => set($num * 2));
   ...
   describe('get', () =>
   {
      ...
      {
         const store = readable('store', 42, () => void 0);
